...
export function mapError(error: unknown): { statusCode: number; code: string; message: string } {
  if (error instanceof MiddlewareError) {
    ...
    if (error.code === 'SCHEMA_INCOMPATIBLE' || error.code === 'DEPENDENCY_UNAVAILABLE') {
...
export class MiddlewareExceptionFilter implements ExceptionFilter {
  constructor(private readonly responseFactory: ResponseFactory) {}
    ...
    const result = this.responseFactory.error(mapped.statusCode, mapped.message, mapped.code, context);
