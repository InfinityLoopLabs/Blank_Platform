...
export type TransportProtocolType = 'http' | 'grpc' | 'ws' | 'unknown'
...
export type GrpcMetadataLikeType = {
...
function readGrpcHeader(metadata: GrpcMetadataLikeType, key: string): string {
  if (metadata.get) {
...
export function isGrpcMetadataLike(
  value: unknown,
): value is GrpcMetadataLikeType {
...
export function buildHttpRequestContext(
  ...
  return {
    ...
    canCreateSample: parseBoolean(
      readHttpHeader(headers, 'x-can-create-sample'),
    ),
  }
}

export function buildWebsocketRequestContext(
  headers?: IncomingHttpHeaders,
): RequestContextType {
  if (!headers) {
...
export function buildGrpcRequestContext(metadata: unknown): RequestContextType {
  ...
  return {
    ...
    correlationId:
      readGrpcHeader(metadata, 'x-correlation-id') || randomHex(16),
    traceId: readGrpcHeader(metadata, 'traceparent') || randomHex(32),
    spanId: readGrpcHeader(metadata, 'x-span-id') || randomHex(16),
    userId,
    isAuthenticated: userId.length > 0,
    canCreateSample: parseBoolean(
      readGrpcHeader(metadata, 'x-can-create-sample'),
    ),
  }
}

export function anonymousRequestContext(
  protocol: TransportProtocolType = 'unknown',
): RequestContextType {
  return {
    protocol,
