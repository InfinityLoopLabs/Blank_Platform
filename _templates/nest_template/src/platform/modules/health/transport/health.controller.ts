...
export class HealthController {
  ...
  liveness(@Req() request: RequestWithContextType) {
    const context = getRequestContext(request)

    return this.responseFactory.ok('liveness up', { status: 'up' }, context)
      .envelope
  }
  ...
  @HttpCode(HttpStatus.OK)
  healthStatus(@Req() request: RequestWithContextType) {
    const context = getRequestContext(request)
