...
export class IntegrationPolicyService {
  ...
  static fromEnv(env: NodeJS.ProcessEnv): IntegrationPoliciesType {
  ...
  static fromRepository(
    ...
    const build = (
      ...
      const enabledRaw = (
        repository.get(`${prefix}_ENABLED`) ?? `${isEnabledByDefault}`
      )
        .toLowerCase()
        .trim()
      ...
      const timeoutRaw = (
        repository.get(`${prefix}_TIMEOUT_MS`) ?? '2000'
      ).trim()
      ...
      return {
        ...
        endpoint: (
          repository.get(`${prefix}_ENDPOINT`) ?? defaultEndpoint
        ).trim(),
        timeoutMs,
        retryCount,
      }
  ...
  static capabilityReport(
    config: IntegrationPoliciesType,
  ): IntegrationHealthType[] {
