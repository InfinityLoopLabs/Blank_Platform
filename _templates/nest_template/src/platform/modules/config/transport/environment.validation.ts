...
export function validateEnvironment(source: RuntimeEnvType): RuntimeEnvType {
  ...
  const isPostgresEnabled = requireBoolean(env, 'POSTGRES_ENABLED')
  const isClickhouseEnabled = requireBoolean(env, 'CLICKHOUSE_ENABLED')
  const isScyllaEnabled = requireBoolean(env, 'SCYLLA_ENABLED')
  const isRedisEnabled = requireBoolean(env, 'REDIS_ENABLED')
  const isKafkaEnabled = requireBoolean(env, 'KAFKA_ENABLED')
  const isMinioEnabled = requireBoolean(env, 'MINIO_ENABLED')
...
function requireNonEmpty(env: RuntimeEnvType, key: string): string {
...
function requireBoolean(env: RuntimeEnvType, key: string): boolean {
...
function requirePositiveInteger(env: RuntimeEnvType, key: string): number {
...
function requireNonNegativeInteger(env: RuntimeEnvType, key: string): number {
  const value = requireNonEmpty(env, key)
  const parsed = Number(value)
  if (!Number.isInteger(parsed) || parsed < 0) {
