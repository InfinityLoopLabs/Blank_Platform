...
async function bootstrap() {
  ...
  const appEnv = (process.env.APP_ENV ?? process.env.NODE_ENV ?? config.appEnv).trim().toLowerCase();
  ...
  app.enableCors({
    origin: corsOrigins.length > 0 ? corsOrigins : appEnv === 'production' ? false : true,
    credentials: true,
  });
  app.useGlobalPipes(
    new ValidationPipe({
  ...
  const isSwaggerEnabled = swaggerFlag ? swaggerFlag === 'true' : appEnv !== 'production';
  if (isSwaggerEnabled) {
